...
dnf --installroot=${ROOTFS} --repofrompath=dockerx,rootfs/etc/yum.repos.d/docker.repo --disablerepo='*' --enablerepo=fedora --enablerepo=updates --enablerepo=dockerx --releasever=${VERSION_ID} --setopt=tsflags=nodocs install -y ${PACKAGES}
